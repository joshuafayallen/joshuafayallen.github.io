...
sns.histplot(data = scaled_y.to_pandas(), x = 'explosive_play_rate_transformed')
...
pair_coach_idx = np.array(
    pd.Categorical(
        ...
        categories=unique_play_callers  
    ).codes
)
...
```{r}
#| label: overall-marginal-lift
#| fig-cap: "Overall Marginal Lift"
...
```{r}
#| label: plot-seans
#| fig-cap: "Sean Mcvay versus Sean Payton Explosive Play Rate by Personnel Groupings"
...
We can also likely explain some of the discrepencies between in explosive play rate just due to overall team strength. If we examine the HSGP for Andy Reid we see that the explosiveness of the Chief's offense is fairly stable throughout his tenure. However, around the 2023 season a lot of analysts and pundits started to ask where did the explosive play element of the offense go? By this time offensive roster was significantly older, the line became a problem, and critically the Chiefs kept missing on pass catchers. 


```{python}
#| code-fold: true
#| label: plot-andy-hsgp
...
While it is difficult to tell from just eyeballing there seems to be some evidence of a lower baseline rate towards the tail of the graph. To test this we are just going to visualize these differences



```{python}
#| echo: false
#| label: make-marginal-contrasts

early_mask = (time_vals < 2023)
recent_mask  = (time_vals > 2023)

early = p_samples[early_mask, :].mean(axis = 0)
recent = p_samples[recent_mask, :].mean(axis = 0)

diff = recent - early 

mu_extracted = idata.posterior['mu'].values
n_chains, n_draws, n_obs = mu_extracted.shape
mu_reid = mu_extracted[:, :, reid_obs]  
p_reid  = expit(mu_reid)                


early_idx = np.where(early_mask)[0]
recent_idx  = np.where(recent_mask)[0]

early_mean  = p_reid[:, :, early_idx].mean(axis=2) 
recent_mean = p_reid[:, :, recent_idx].mean(axis=2)   
...
early_flat = early_mean.flatten()
recent_flat = recent_mean.flatten()

diff_df = pl.DataFrame({
    'chain': chain_ids,
    'draw':  draw_ids,
    'diff':  diff_flat, 
    'pre_2023_mean': early_flat, 
    'post_2023_mean': recent_flat
})

diff_df.write_csv('blog-post-data/reid-contrasts.csv')
...
```{r}
#| fig-cap: "Posterior Constrasts From 2017-2022 and 2023 on"
#| label: plot-marginal-contrasts

reid_post_contrasts = read_csv('blog-post-data/reid-contrasts.csv') 

kc_red = nflplotR:::primary_colors['KC']
kc_second = nflplotR:::secondary_colors['KC']

ggplot(reid_post_contrasts, aes(x = diff)) + 
    stat_halfeye(point_interval = 'median_hdi', slab_alpha = 0.5, color = kc_red, fill = kc_second) + 
    geom_vline(xintercept = 0) +
    labs(x = 'Posterior Contrast', y = NULL) + 
    theme_minimal()


```

We do see that there is a small difference in explosive play rate but altogether less than a full percent and the bulk of the distribution is below zero. So altogether not an overwhelming amount of evidence that their is a huge difference. 

## Concluding Thoughts

After a rather long blog post and a project that, admittedly, took longer than anticipated we have a workingish MMM. When trying to understand why these effects might be small there was a lot that I learned about MMM's and a lot that I learned football. I kept coming back to the plot below.



```{python}
#| echo: false
#| label: make-plotting-data

idata_prior = (pl.from_pandas(
    idata.prior[['mm_alpha', 'mm_lam']].to_dataframe().reset_index())
    .with_columns(pl.col('coach_group_pairs').str.split_exact('|',1)
        .struct.rename_fields(['coach', 'personnel_grouping']),
        pl.lit("prior").alias('group'))
        .unnest('coach_group_pairs')
)


idata_post = (pl.from_pandas(
    idata.posterior[['mm_alpha', 'mm_lam']].to_dataframe().reset_index())
    .with_columns(pl.col('coach_group_pairs').str.split_exact('|',1)
        .struct.rename_fields(['coach', 'personnel_grouping']),
        pl.lit('posterior').alias('group'))
    .unnest('coach_group_pairs')
)


all_together =  pl.concat([idata_prior, idata_post], how = 'diagonal_relaxed')

all_together.write_parquet('blog-post-data/sat-prior-v-post.parquet', compression='zstd')


```

```{r}
#| fig-cap: "Prior vs Posterior for Michaelis Menten Saturation Parameters"
#| label: post-v-prior-saturation

raw_sat_dat = read_parquet('blog-post-data/sat-prior-v-post.parquet') 

clean_sat = raw_sat_dat |>
    pivot_longer(
        cols = c(mm_alpha:mm_lam), 
        names_to = 'param', 
        values_to = 'val'
    ) |>
        filter(coach %in% c('Sean Payton')) |>
    mutate( digits = str_extract(personnel_grouping, '\\d{2}'),
            labs_nice = glue::glue("{digits} Personnel"), 
            group = factor(group, levels = c('prior', 'posterior')))


ggplot(clean_sat, aes(x = val, y = labs_nice, color = group)) +
    stat_halfeye(position = position_dodge(width = 0.5), point_interval = 'median_hdi') +
    facet_wrap('param') + 
    labs(y = NULL) + 
    scale_fill_met_d('Lakota') + 
    theme_minimal()
...
When going through my model checks I noticed that the priors and the posteriors for the saturation parameters mostly didn't change. There are some slight shifts or the HDIs get a bit tighter but overall it looked like the saturation was being mostly driven by my priors. Through lots and lots changing priors or switching saturation functions I found that nothing really changed these results. The saturation did not really update at all when I introduced data into the model. 

To be honest I was pretty stumped so I just decided to plot the usage data against the simulated data. I just wanted to get a sense of what an idealized example of adspend looks like against some data that aren't typically used for MMMs.  



```{python}
#| label: plot-epr-v-sim
#| fig-cap: "Explosive Play Rate Versus Simulated Data"
#| code-fold: true
...
long_personnel = (
    raw_data
    .select(cs.starts_with('personnel'), pl.col('off_play_caller'))
    .unpivot(index = 'off_play_caller')
    .with_columns(
        pl.col('variable').str.extract(r"(\d{2})").str.to_integer().alias('digits')
    )
    .filter(pl.col('digits').is_in([11,12,21,22]))
)

fig, axs = plt.subplots(ncols = 2)

sns.kdeplot(long_fake_data, x = 'value',  hue = 'variable', ax = axs[0])
sns.kdeplot(long_personnel, x = 'value', hue = 'variable', ax = axs[1])

axs[0].set_title('Distribution of Fake Sales Data')
axs[1].set_title('Distribution of Personnel Usage')

```


What stands out wen we compare the distributions of our channel variables is that adspend is pretty variable, whereas our personnel usage is much less variable outside of 11 and 12 personnel. This makes sense since 4 of the 9 play callers are either Sean McVay, Kyle Shanahan, or worked for one or both of them.^[Notably Sean McVay was a TE coach under Kyle Shanhan in 2013 when Kyle Shanahan was offensive coordinator for his dad Mike Shanahan] So their approach to offensive football is going to pretty similar. 

This got me thinking about some posts I get from time to time from one of the founders of Recast on LinkedIn. Recast is an analytics firm that has built out a whole analytics suite to help brands track their return on adspend. Across a few posts he kind of distills why we would likely expect the results we got. The gist of these posts boil down to effectively this graph. Basically, MMM's not only require some level spend across channels but we do also require some noticeable levels of variation. Companies can build this in across ad campaigns by just varying your spend across channels. This gives the model more variation to latch onto a update the posterior accordingly. 

Purposely throttling down usage of one personnel grouping from week-to-week does happen for some coaches, but in all likelihood the extent that it is needed to get any additional signal for each play caller likely doesn't happen. Who is available, who the opponent is, and the game script are going to do a fair amount of heavy lifting for what is going to dictate personnel usage in the game and what gets installed that week. But these changes are going to be pretty meager in comparision to what is needed to get a real signal from the data. 