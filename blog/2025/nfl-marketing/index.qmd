...
df["x1_adstock"] = geometric_adstock(
    x=df["x1"].to_xarray(), alpha=alpha1, l_max=8, normalize=True
).eval()

df["x2_adstock"] = geometric_adstock(
    x=df["x2"].to_xarray(), alpha=alpha2, l_max=8, normalize=True
).eval()
...
```{python}
long_effects = pl.from_pandas(df).unpivot(on = cs.starts_with('x'), index = 'date_week')
