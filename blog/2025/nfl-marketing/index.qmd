...
raw1 = pl.read_parquet('blog-post-data/pbp-data1.parquet')
raw2 = pl.read_parquet('blog-post-data/pbp-data2.parquet')

raw_nfl = pl.concat([raw1, raw2], how = 'diagonal_relaxed')
...
raw_data = pl.concat([raw1, raw2], how = 'diagonal_relaxed')
...
Borrowing some work from [Andrew Heiss's blog post](https://www.andrewheiss.com/blog/2023/09/18/understanding-dirichlet-beta-intuition/) and simulate some data out of a Dirichlet(3,7,2) where the probability of being in the zero part is roughly 25%, the probability of being between 0 and 1 is roughly 57%, and the probability of being 1 is roughly 17%. 
...
What we care about in the Ordered Beta setting is the boundaries between zero values greater than 0 and less than one and the boundary before 