...
As a good Bayesian lets take a look at whether the model sampled well. Normally we would just load in a saved model from a netcdf file and then just read in everything using `az.from_netcdf` and then just access everything more pythonically. Unfortunately, these netcdf files are massive and GitHub has a pretty strict file size limitation even with git large file storage. So we have to use a few parlor tricks to get around this limitation. 
