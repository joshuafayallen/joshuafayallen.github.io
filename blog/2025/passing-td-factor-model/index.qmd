...
description: "How Good is Your Favorite Pass Catcher?"
categories:
  - r
  - python
  - bayes
...
empirical_dat = read_parquet(here::here('writeup-dat', 'cleaned-data.parquet')) |>
  # unclear what is going on but there are two 6 td games
  # one by Kevin Kasper who had 2 tds that game 
  # and Randy moss
  mutate(
    rec_tds_game = case_when(
      receiver_full_name == 'Kevin Kasper' & rec_tds_game == 6 ~ 2,
      receiver_full_name == 'Randy Moss' & rec_tds_game == 6 ~ 2,
      .default = rec_tds_game
    )
  )
...
The variance for the HSGPs are set by really looking at the observed proportions of TDs. I set it by looking at the proportion of 2-touchdown games. So, roughly what is the proportion of two-touchdown games we would expect to observe? In the actual data, there is about a 2% chance of a player having a two-touchdown game. I found that depending on how you subset the data changing this parameter from 2.1% to 3% can make a pretty big difference with how the model is sampling. In addition, if you feed it a subset of data you are going to need to change how much the slope varies. 
...
cumulative_stats = (
    pl.read_parquet("writeup-dat/cleaned-data.parquet")
    .with_columns(
        pl.when(
            (pl.col("receiver_full_name") == "Kevin Kasper")
            & (pl.col("rec_tds_game") == 6)
        )
        .then(2)
        .when(
            (pl.col("receiver_full_name") == "Randy Moss")
            & (pl.col("rec_tds_game") == 6)
        )
        .then(2)
        .otherwise(pl.col("rec_tds_game"))
        .alias("rec_tds_game")  # or a new name if you prefer
    )
)
...
As a good Bayesian we always need to look at whether the model sampled well. With some pretty permissive settings we still get one divergence which is not the end of the world, but we would like it to be zero.

```{python}
#| message: true
#| label: readin-post-preds




idata = az.from_netcdf('writeup-dat/post-preds.nc')

idata.sample_stats.diverging.sum().data

```

Most if not all of our proposals were accepted which is good! 
...
read_parquet('writeup-dat/rhats.parquet') |>
  mutate(Rhat = round(`0`, 2)) |>
  select(-`0`) |>
  tinytable::tt()
...
Personally, I would have really liked everybody to be above 1,000, but each parameter is well above the threshold where `PyMC` will yell at you which is good!
...
f_within_post = xr.open_dataarray('writeup-dat/games-hsgp.nc')
f_long_post = xr.open_dataarray('writeup-dat/seasons-hsgp.nc')
f_total_post = xr.open_dataarray('writeup-dat/total-hsgp.nc')

some_draws = rng.choice(f_total_post.draw, size = 100, replace = True)
...
axes["A"].plot(
    ...
    az.extract(f_within_post)["f_games"].isel(sample=0),
    color="#70133A",
    alpha=0.3,
    lw=1.5,
    label="random draws",
...
axes["A"].plot(
    f_within_post.gameday,
    az.extract(f_within_post)["f_games"].isel(sample=some_draws),
    color="#70133A",
    alpha=0.3,
    lw=1.5,
)
...
g = ggplot(games_hsgp, aes(x = timestamp, y = f_games)) +
  geom_smooth(
    aes(group = .iteration),
    se = FALSE,
    alpha = 0.3,
...
s = ggplot(seasons_hsgp, aes(x = timestamp, y = f_season)) +
  geom_smooth(
    aes(group = .iteration),
    se = FALSE,
    alpha = 0.3,
...
t = ggplot(total_hsgp, aes(x = timestamp, y = td_probs)) +
  geom_smooth(
    aes(group = .iteration),
    se = FALSE,
    alpha = 0.3,
...
PAR = xr.open_dataarray('writeup-dat/par-season.nc')
...
players_stats = read_parquet('writeup-dat/player-stats.parquet')
...
#
#
#posterior_predictive = open_dataset('posterior-predictive') |>
#  collect() 
#
...
#
#write_parquet(par_df, 'writeup-dat/par-df-season.parquet')

par_df = read_parquet('writeup-dat/par-df-season.parquet')
...
ggplot(
  ...
  labs(y = NULL, x = 'Performance Above Replacement') +
...
PAR = xr.open_dataarray('writeup-dat/par-sans-season.nc')
...
#write_parquet(par_df, 'writeup-dat/par-sans-seasons.parquet')

par_df = read_parquet('writeup-dat/par-sans-seasons.parquet')
...
One interesting thing that we can look at is not only how do players evolve as pass catchers overtime, but also how does switching from a bad team to a good team affect your production. If you are an Eagles fan you could totally look at what Saquon Barkely did as an Eagle last year. However, I decided to go with Christian McCaffrey because among active RBs he is the leader in career receiving TDs. I subset the data to only include his healthy seasons to get a cleaner comparision of his time as a Niner and his time as a Panther.
...
player_probs_post = xr.open_dataarray('writeup-dat/cmc-implied-probs.nc').assign_coords(cmc_coords)

colors = plt.cm.viridis(np.linspace(0.05, 0.95, 4))

cols = 2
unique_seasons = np.array([2017,2018, 2019, 2022,2023])
...
axes[3].annotate(
    ...
    xytext=(10, 0.50),  # Position of the text
    xycoords="data",
    textcoords="data",
...
```{r}
#| code-fold: true
#| label: amon-ra-plot-r



cmc = read_parquet('writeup-dat/cmc-implied-probs.parquet')

line_df = data.frame(
  games_played = 7,
  tds_scored_probs = 0.50,
  season = 2022,
  n_tds = NA
...
ggplot(cmc, aes(x = games_played, y = tds_scored_probs, fill = n_tds)) +
  stat_lineribbon(.width = (0.95)) +
  scale_fill_met_d(name = 'Troy') +
  facet_wrap(vars(season), scales = 'free') +
  geom_vline(data = line_df, aes(xintercept = games_played), linetype = 'dashed') +
  geom_text(data = ann_text, aes(label = lab, x = games_played, y = tds_scored_probs), size = 2.5, inherit.aes = FALSE) +
  labs(
...
He puts up a really impressive rookie campaign with the probablity that he scores at least one TD hovering around 0.25 for the entiriety of his rookie campaign. He wouldn't match this kind of projected production until he became a Niner.
...
#  filter(player_display_name %in% c(all_timers, active_player), position_group %in% c(
#    'RB', 'TE', 'WR'
#  )) |>
#    select(player_display_name, position_group) |>
#    distinct(player_display_name, .keep_all = TRUE) |>
#    rename(receiver_full_name = player_display_name)
#
#
#implied_probs_data = open_dataset('implied-probs') |>
#  filter(receiver_full_name %in% c(all_timers, active_player)) |>
#  collect() |>
#  mutate(n_tds = as.factor(event)) |>
# group_by(receiver_full_name, n_tds) |>
#   median_qi(tds_scored_probs, .width = c(0.65, 0.95 )) |>
...
#     
#table(implied_probs_data$receiver_full_name)
#
#write_parquet(implied_probs_data,'writeup-dat/modern-v-retired.parquet')

plot_data = read_parquet('writeup-dat/modern-v-retired.parquet')
