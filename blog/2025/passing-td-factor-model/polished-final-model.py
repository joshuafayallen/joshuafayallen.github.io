...
implied_probs_small = implied_probs.filter(
    ...
    pl.when(
        pl.col('receiver_full_name').is_in(all_timers)
    )
    .then(pl.lit('Retired Player'))
    .otherwise(pl.lit('Active Player')).alias("active")
).collect()


implied_probs_small.filter(
    pl.col('active') == 'Active Player'
).write_parquet(
    'writeup-dat/active-players-implied-probs.parquet',
    use_pyarrow=True
)
...
implied_probs_small.filter(
    ...
    'writeup-dat/retired-players-implied-probs.parquet',
    use_pyarrow=True
)

player = "George Kittle"

just_cmc = cumulative_stats_pd[cumulative_stats_pd['receiver_full_name'] == player]
cmc_coords = xr.Coordinates.from_pandas_multiindex(
    just_cmc.set_index(
        [
            "receiver_full_name",
            "number_of_seasons_played",
...
player_probs_post.reset_index('obs_id').to_netcdf('writeup-dat/gk-implied-probs.nc')


player_probs_post = xr.open_dataarray('writeup-dat/gk-implied-probs.nc').assign_coords(cmc_coords)
...
some_draw = rng.choice(idata.posterior_predictive.draw, size = 100, replace = True)

small = idata.posterior_predictive.isel(draw = some_draw)



post_preds_save = az.InferenceData(
    posterior_predictive = small.reset_index('obs_id'),
...
az.plot_ppc(post_preds_save)


post_preds_save.to_netcdf(
    'writeup-dat/post-preds.nc'
)
...
seasons = range(2002,2025)

def pb_upload(season):
    print(f"writing data for {season} season")
    d = implied_probs.filter(
        pl.col('season') == season
    ).collect()
    d.write_parquet(f"pb-upload/implied-probs-{season}-season.parquet",
                    use_pyarrow=True, pyarrow_options={'compression':'zstd'})
    return print(f" done writing data for {season} season")
